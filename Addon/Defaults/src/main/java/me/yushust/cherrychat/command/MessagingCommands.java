...
public class MessagingCommands {
    ...
    public MessagingCommands(CherryChatStoraging plugin) {
        this.plugin = plugin.getChatPlugin();
        this.userDataHandler = plugin.getUserDataHandler();
    }

    public CherryCommand messageCommand = (sender, args) -> {
        ...
        if(target instanceof Player && sender instanceof Player) {
            ...
            User targetUser = userDataHandler.findSync(playerTarget.getUniqueId());
            ...
            User senderUser = userDataHandler.findSync(playerSender.getUniqueId());
    ...
    public CherryCommand replyCommand = (sender, args) -> {
        if(args.length < 1) {
